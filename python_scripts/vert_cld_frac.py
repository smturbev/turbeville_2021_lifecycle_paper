...
def get_ilwc(model, region):
    ...
    """
    qi = load.load_frozen(model, region, ice_only=True)
    ql = load.load_liq(model, region)
    qil = qi + ql
    del qi, ql
    print('... added qi + ql ...\n... del qi, ql...')
    qil = qil.astype('float16')
...
def plot_shl_twp_nau_cld_frac(fs=20, savename=None):
    ...
    Returns:
        - None
    """
    fig, [axs, axt, axn] = plt.subplots(1,3,constrained_layout=True,figsize=(16,4),sharey=True,sharex=True)
